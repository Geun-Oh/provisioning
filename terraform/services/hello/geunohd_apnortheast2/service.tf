...
module "hello" {
	...
	# VPC Inform via remote_state
	shard_id = data.terraform_remote_staet.vpc.outputs.shard_id
	...
	target_vpc = data.terraform_remote_state.vpc.outputs.target_vpc 
	...
	# e.g) hello.geunoh-aws-provisioning.com
	domain_name = "hello"

	# Route53 variables
	acm_external_ssl_certificate_arn = var.r53_variables.id.star_geunohawsprovisioning_com_acm_arn_apnortheast2
	route53_external_zone_id = var.r53variables.id.geunohawsprovisioning_com_zone_id
